...
| Notch column wider (1.25 : 1) | Rig column wider (0.44 / 0.56) | Reversed on the owner's reading of the running app. The notch column is only ever as tall as the number of notches, so extra WIDTH there buys nothing, while the rig column's routing table was scrolling sideways beside it. |
| Averaging: off / 1 s / 3 s | off / 0.1 / 0.3 / 0.5 / 1 s as segments, 3 / 5 / 10 s in a combo | Chasing a ring needs sub-second averaging within reach; long averages are for reading a room and are never urgent. |
| Type sizes as measured in a browser | Section caption 14.5, column 13, switch legend 21, brand 16 | Saira Condensed is a CONDENSED face and renders both smaller and lighter in the app than the study's browser did. The study fixes the RATIOS; the absolute sizes have to suit the face actually being drawn. |
| Empty states in the body face | Legend face, tracked uppercase | They are labels for an empty table, sitting among tracked legends and monospaced numbers. Set as prose they were the only sentence on screen and read as a stray piece of another design. |
| Masthead: rig line + protection badge | Also a device chip and the audio callback's CPU share | "Is the interface there?" and "is it filtering?" are different questions, and a stopped device showing BYPASSED looked like a choice rather than a fault. CPU is the callback's share of its budget -- the figure that predicts a dropout -- not the process's share of the machine. |
