...
| M | **Soundcheck đo chủ động**: chirp/MLS qua từng output, đo loa→mic, loop gain theo tần số, notch trước khi hú | Có (phát tín hiệu test ra PA) | S (đo per-output = per-lane) | [`2026-09-15-active-soundcheck-design.md`](2026-09-15-active-soundcheck-design.md) — **đã triển khai trên nhánh, chờ owner duyệt 9+ mục và nghe trên rig** |
...
| M | **đã triển khai trên nhánh** `feat/lane-m-active-soundcheck` (`a6be099..17f5225`, 34 commit, 11 task SDD + fix rounds, suite **712/712**), spec [`2026-09-15-active-soundcheck-design.md`](2026-09-15-active-soundcheck-design.md) **rev 5** (§9 = bảng sai lệch khi triển khai), sổ quyết định Q1–Q23: sweep log 100 Hz–10 kHz, −20 dBFS đỉnh, 3,0 s mỗi kênh ngõ ra, kênh đang đo **im hoàn toàn 4,52 s** (~72,3 s xấu nhất cho 16 kênh), loop gain `H_dB` theo bin với `margin = −H_dB`, băng tin cậy dừng ở **6 kHz**, chín lý do tự hủy, **chỉ ĐỀ XUẤT** cho tới khi bấm `ÁP DỤNG`, dọn theo sổ per-slot, năm sự kiện `soundcheck_*` trong log. **CHƯA merge, CHƯA đóng gói** — chờ owner duyệt **9 mục cũ + 6 mục Q18–Q23** và **một lần nghe trên rig**: đường `ÁP DỤNG` không tới được trong headless, nên lần `ÁP DỤNG` thật đầu tiên sẽ xảy ra trên một dàn thật | 2026-09-16 |
